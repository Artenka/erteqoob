...
$(document).ready(function () {
  ...
  var table = $('#table_courses').DataTable({
    ajax: {
    ...
    columnDefs: [
      {
        ...
        render: function (data, type, full, meta) {
          return '<a href="/admin/courses/' + full[0] + '">' + data + '</a>'
        }
      }
    ],
    order: [2, 'desc'],
...
function priorityAction(id, action) {
  ...
  $.get(window.location.origin + "/admin/courses/actions", data, function(resp) {
    if (resp.result) {
...
function deleteItemBack(selectedId) {
  ...
  $.get(window.location.origin + "/admin/courses/" + selectedId + "/delete", function (resp) {
    if (resp.result) {
      table_all_global.ajax.reload();
    } else {
      if (resp.error) {
